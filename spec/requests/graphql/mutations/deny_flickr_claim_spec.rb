...
RSpec.describe 'denyFlickrClaim Mutation', type: :request do
  ...
  context 'when the user is not an admin' do
    ...
    it 'returns not authorized error and no claim' do
      ...
      expect(payload['success']).to be(false)
      expect(payload['claim']).to be_nil
      expect(payload['errors']).to include('Not authorized to deny claims')
    end
  end
  ...
  context 'when denial succeeds' do
    let(:admin) { create(:user, admin: true) }
    ...
    it 'returns success and denies the claim' do
      post_mutation
      payload = data_dig(response, 'denyFlickrClaim')
      ...
      expect(claim_payload['status']).to eq('denied')
      expect(claim_payload['deniedAt']).to be_present
      expect(claim.reload.status).to eq('denied')
