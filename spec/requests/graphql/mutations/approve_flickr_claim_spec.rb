...
RSpec.describe 'approveFlickrClaim Mutation', type: :request do
  ...
  context 'when the user is not logged in' do
    it 'returns an error' do
      ...
      result = json['data']['approveFlickrClaim']
  ...
  context 'when a regular user is logged in' do
    ...
    it 'returns an authorization error' do
      ...
      expect(result['claim']).to be_nil
      expect(result['success']).to be false
      expect(result['errors']).to include('Not authorized to approve claims')
    end
  end
  ...
  context 'when an admin is logged in' do
    ...
    context 'when the claim is valid' do
      it 'approves the claim and associates the Flickr user with the claiming user' do
        post_mutation
        ...
        expect(result['claim']['flickrUser']['claimedByUser']['id']).to eq(user.slug)
    ...
    context 'when approving an automatic claim' do
      let(:claim) { create(:flickr_user_claim, :automatic, user: user, flickr_user: flickr_user) }

      it 'successfully approves the claim' do
        post_mutation
    ...
    context 'when approving a manual claim' do
