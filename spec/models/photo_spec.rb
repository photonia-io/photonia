...
RSpec.describe Photo do
  ...
  describe 'user thumbnail' do
    describe '#user_thumbnail?' do
      context 'when user_thumbnail is present' do
        let(:photo) { build_stubbed(:photo, user_thumbnail: { 'top' => 0.25, 'left' => 0.25, 'width' => 0.5, 'height' => 0.5 }) }

        it 'returns true' do
          expect(photo.user_thumbnail?).to be true
        end
      end

      context 'when user_thumbnail is nil' do
        let(:photo) { build_stubbed(:photo, user_thumbnail: nil) }

        it 'returns false' do
          expect(photo.user_thumbnail?).to be false
        end
      end
    end

    describe '#thumbnail_source' do
      let(:user_thumbnail_data) { { 'top' => 0.1, 'left' => 0.1, 'width' => 0.3, 'height' => 0.3, 'x' => 100, 'y' => 100, 'pixel_width' => 300, 'pixel_height' => 300 } }
      let(:intelligent_thumbnail_data) { { top: 0.25, left: 0.25, width: 0.5, height: 0.5, x: 250, y: 250, pixel_width: 500, pixel_height: 500 } }

      context 'when user_thumbnail exists' do
        let(:photo) { build_stubbed(:photo, user_thumbnail: user_thumbnail_data) }

        it 'returns user_thumbnail' do
          expect(photo.thumbnail_source).to eq(user_thumbnail_data)
        end
      end

      context 'when only intelligent_thumbnail exists' do
        let(:photo) { build_stubbed(:photo, user_thumbnail: nil) }

        before do
          allow(photo).to receive(:intelligent_thumbnail).and_return(intelligent_thumbnail_data)
        end

        it 'returns intelligent_thumbnail' do
          expect(photo.thumbnail_source).to eq(intelligent_thumbnail_data)
        end
      end

      context 'when neither thumbnail exists' do
        let(:photo) { build_stubbed(:photo, user_thumbnail: nil) }
        ...
        it 'returns nil' do
          expect(photo.thumbnail_source).to be_nil
        end
      end
    end
  end
