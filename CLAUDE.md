...
Photonia: self-hosted photo sharing. Rails 8 (Ruby 3.4.11) API + Vue 3 SPA, Postgres, Sidekiq/Redis, Shrine on S3, AWS Rekognition auto-tagging. Most photos were imported from a Flickr export.
...
System specs run with the suite through Cuprite and need a local Chrome/Chromium (`sudo apt install chromium`); in CI they run only via the manual "System specs" workflow (`system-specs.yml`).
