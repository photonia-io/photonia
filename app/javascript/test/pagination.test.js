...
test("Pagination renders the correct elements", () => {
  ...
  // check if the "< Prev" button leads to page 1
  ...
  expect(wrapper.findAllComponents(RouterLinkStub)[2].props().to).toEqual({
    name: "photos-index",
  });

  // check if the last page in the list is 10
  expect(wrapper.findAllComponents(RouterLinkStub)[7].props().to).toEqual({
    name: "photos-index",
    query: { page: 10 },
  });
